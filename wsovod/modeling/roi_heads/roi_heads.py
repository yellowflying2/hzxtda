...
from wsovod.modeling.test_time_adaptation import update_cache, compute_cache_logits, softmax_entropy, get_entropy
...
class WSOVODROIHeads(ROIHeads):
    ...
    def __init__(
        ...
        *,
        ...
        metadata: Dict = None,
        use_tda: bool = False,
        pos_cache_config: dict = None,
        neg_cache_config: dict = None,
        **kwargs,
    ):
        """
        NOTE: this interface is experimental.
        ...
        self.pos_cache_config = pos_cache_config or {"enabled": False}
        self.neg_cache_config = neg_cache_config or {"enabled": False}
        self.pos_cache = {}
        self.neg_cache = {}
    ...
    def from_config(cls, cfg, input_shape):
        ...
        if inspect.ismethod(cls._init_box_head):
            ...
            # TDA configuration
        ret["use_tda"] = cfg.MODEL.ROI_HEADS.get("USE_TDA", False)
        ret["pos_cache_config"] = {
            "enabled": cfg.MODEL.ROI_HEADS.get("POS_CACHE_ENABLED", False),
            "shot_capacity": cfg.MODEL.ROI_HEADS.get("POS_CACHE_CAPACITY", 5),
            "alpha": cfg.MODEL.ROI_HEADS.get("POS_CACHE_ALPHA", 1.0),
            "beta": cfg.MODEL.ROI_HEADS.get("POS_CACHE_BETA", 1.0),
        }
        ret["neg_cache_config"] = {
            "enabled": cfg.MODEL.ROI_HEADS.get("NEG_CACHE_ENABLED", False),
            "shot_capacity": cfg.MODEL.ROI_HEADS.get("NEG_CACHE_CAPACITY", 5),
            "alpha": cfg.MODEL.ROI_HEADS.get("NEG_CACHE_ALPHA", 1.0),
            "beta": cfg.MODEL.ROI_HEADS.get("NEG_CACHE_BETA", 1.0),
            "entropy_threshold": {
                "lower": cfg.MODEL.ROI_HEADS.get("NEG_ENTROPY_LOWER", 0.3),
                "upper": cfg.MODEL.ROI_HEADS.get("NEG_ENTROPY_UPPER", 0.7),
            },
            "mask_threshold": {
                "lower": cfg.MODEL.ROI_HEADS.get("NEG_MASK_LOWER", 0.2),
                "upper": cfg.MODEL.ROI_HEADS.get("NEG_MASK_UPPER", 0.8),
            },
        }
    ...
    def _forward_box(
        ...
        if self.training:
            ...
            # Refine predictions with TDA if enabled
            if self.refine_K > 0:
                ...
                if self.use_tda:
                    ...
                    confidences = F.softmax(final_scores, dim=1)
                    pred_classes = confidences.argmax(dim=1)
                    ...
                    if pos_enabled and self.pos_cache:
                        adjusted_scores += compute_cache_logits(
                            box_features, 
                            self.pos_cache, 
                    ...
                    if neg_enabled and self.neg_cache:
                        adjusted_scores -= compute_cache_logits(
                            ...
                            self.num_classes,
                            (self.neg_cache_config["mask_threshold"]["lower"], self.neg_cache_config["mask_threshold"]["upper"])
                        )
